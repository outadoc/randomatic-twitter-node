        ...
        getRandomTweet: function(tweets, data, callback) {
            ...
            if (randomWord != undefined) {
                for (tweet = randomWord + " ";;) {
                    randomWord: {
                        randomWord = tweets[uppercase ? randomWord : randomWord.toUpperCase()];
                        var c = Math.random(),
                            g = 0,
                            e = void 0;
                        for (e in randomWord) {
        ...
        getNewTweet: function (data, twitterObj, callback) {
            if (t[data.user.screen_name]) {
                ...
                twitterObj.get('/statuses/user_timeline.json', {screen_name: encodeURIComponent(data.user.screen_name), count: 80}, function(error, tweets) {
                    if(error) {
                        module.exports.callback("error getting " + data.user.screen_name + "'s timeline: " + error, null);
                    } else {
